...
function _filesystem.is_directory_readable(path, callback)
  _filesystem.query_info(path, function(info)
    if info ~= nil then
      if
        info:get_file_type() == "DIRECTORY"
        and info:get_attribute_boolean("access::can-read")
      then
        callback(true)
      else
    ...
    if not is_readable then
      ...
      gfile:create_readwrite_async(
        ...
        function(_, create_result)
          print(
            "file created "
              .. tostring(gfile:create_readwrite_finish(create_result))
          )
      ...
      gfile:open_readwrite_async(
        Glib.PRIORITY_DEFAULT,
        nil,
        function(_, io_stream_result)
          ...
          file:write_all_async(
            text,
            Glib.PRIORITY_DEFAULT,
            nil,
            function(_, write_result)
              ...
              file:close_async(
                Glib.PRIORITY_DEFAULT,
                nil,
                function(_, file_close_result)
                  print(
                    "output stream closed "
                      .. tostring(file:close_finish(file_close_result))
                  )
                  io_stream:close_async(
                    Glib.PRIORITY_DEFAULT,
                    nil,
...
function _filesystem.remote_watch(
  path,
  uri,
  interval,
  callback,
  old_content_callback
)
      ...
      callback = function()
        _filesystem.query_info(path, function(info)
          if info ~= nil then
            ...
            local diff = math.ceil(
              Glib.DateTime.new_now_local():difference(time) / 1000000
            )
