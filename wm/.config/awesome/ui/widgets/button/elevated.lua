...
local function button(args)
  ...
  widget.animation = animation:new({
    ...
    update = function(self, pos)
      if pos.color then
        widget:get_children_by_id("background_role")[1].bg =
          helpers.color.rgb_to_hex(pos.color)
      end
      if pos.border_width then
        widget:get_children_by_id("background_role")[1].border_width =
          pos.border_width
      end
...
function elevated_button.state(args)
  ...
  args.hover_bg = args.hover_bg
    or helpers.color.button_color(args.normal_bg, 0.1)
  args.press_bg = args.press_bg
    or helpers.color.button_color(args.normal_bg, 0.2)

  args.on_normal_bg = args.on_normal_bg or args.press_bg
  args.on_hover_bg = args.on_hover_bg
    or helpers.color.button_color(args.on_normal_bg, 0.1)
  args.on_press_bg = args.on_press_bg
    or helpers.color.button_color(args.on_normal_bg, 0.2)

  args.normal_shape = args.normal_shape
    or helpers.ui.rrect(beautiful.border_radius)
  ...
  args.on_normal_border_color = args.on_normal_border_color
    or beautiful.transparent
  args.on_hover_border_color = args.on_hover_border_color
    or beautiful.transparent
  args.on_press_border_color = args.on_press_border_color
    or beautiful.transparent
  ...
  widget:connect_signal(
    "button::release",
    function(self, lx, ly, button, mods, find_widgets_result, fake)
      widget.button = nil

      if button == 1 then
        if
          args.on_turn_on ~= nil
          or args.on_turn_off ~= nil
          or args.on_press
        then
          if widget._private.state == true then
            effect(
      ...
      elseif button == 3 then
        if args.child and args.child.on_secondary_release ~= nil then
          args.child:on_secondary_release(
            self,
            lx,
            ly,
            button,
  ...
  widget:connect_signal("mouse::leave", function(self, find_widgets_result)
    if widget.button ~= nil then
      ...
      elseif widget.button == 3 then
        if
          args.on_secondary_release ~= nil or args.on_secondary_press ~= nil
        then
          effect(
            widget,
            args.normal_bg,
