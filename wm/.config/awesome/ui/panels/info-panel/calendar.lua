...
local function date_widget(date, is_current, is_another_month)
  ...
  return wibox.widget({
    ...
    widgets.text({
      halign = "center",
      size = beautiful.font_size_small,
      color = text_color,
      text = date,
    }),
  })
...
function calendar:set_date(date)
  ...
  local first_day =
    os.date("*t", os.time({ year = date.year, month = date.month, day = 1 }))
  local last_day = os.date(
    "*t",
    os.time({ year = date.year, month = date.month + 1, day = 0 })
  )
  ...
  local days_to_add_at_month_end = 42
    - last_day.day
    - days_to_add_at_month_start

  local previous_month_last_day = os.date(
    "*t",
    os.time({ year = date.year, month = date.month, day = 0 })
  ).day
  ...
  for day = 1, month_days do
    local is_current = day == current_date.day
      and date.month == current_date.month
