...
local empty_notifbox = require(
  "ui.panels.notification-panel.notif-center.build-notifbox.empty-notifbox"
)
local notifbox_scroller = require(
  "ui.panels.notification-panel.notif-center.build-notifbox.notifbox-scroller"
)
...
local notifbox_add = function(n, notif_icon, notifbox_color)
  if
    #notif_core.notifbox_layout.children == 1
    and notif_core.remove_notifbox_empty
  then
  ...
  local notifbox_box = require(
    "ui.panels.notification-panel.notif-center.build-notifbox.notifbox-builder"
  )
  notif_core.notifbox_layout:insert(
    1,
    notifbox_box(n, notif_icon, n.title, n.message, n.app_name, notifbox_color)
  )
end
...
naughty.connect_signal("request::display", function(n)
  ...
  if not notif_icon then
    notif_icon = beautiful.theme_assets.awesome_icon(
      24,
      beautiful.color8,
      beautiful.widget_bg
    )
