	...
	local cpuinfo = wibox.widget({
		{
			{
				id = "watch_role",
				awful.widget.watch([[bash -c 'grep '^cpu.' /proc/stat']], 1, function(widget, stdout)
					for line in stdout:gmatch("[^\r\n]+") do
						if line:sub(1, #"cpu") == "cpu" then
							...
							local diff_idle = idle
								- tonumber(cpus[name]["idle_prev"] == nil and 0 or cpus[name]["idle_prev"])
							local diff_total = total
								- tonumber(cpus[name]["total_prev"] == nil and 0 or cpus[name]["total_prev"])
					...
					widget:set_markup_silently(string.format(" :%.f%%", cpus["cpu"]["diff_usage"]))
				end),
