	...
	local meminfo = wibox.widget({
		{
			{
				id = "watch_role",
				awful.widget.watch([[bash -c 'grep '^Mem.' /proc/meminfo']], 1, function(widget, stdout)
					...
					mem["MemUsed"] = tonumber(mem["MemTotal"] == nil and 0 or mem["MemTotal"])
						- tonumber(mem["MemAvailable"] == nil and 0 or mem["MemAvailable"])
					widget:set_markup_silently(string.format(" :%.f%%", mem["MemUsed"] / mem["MemTotal"] * 100))
				end),
