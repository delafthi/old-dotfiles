  ...
  capi.mousegrabber.run(function(_mouse)
    ...
    for _, v in ipairs(_mouse.buttons) do
      if v then
        ...
        c.screen.selected_tag.master_width_factor = math.min(math.max(
                                                                 new_mwfact,
                                                                 0.01), 0.99)
...
local function twopane_group(gs, cls, wa, orientation, fact, group, useless_gap)
  ...
  for c = group.first, group.last do
    ...
    geom[y] = coord
    gs[cls[c]] = geom
    hints.width, hints.height = apply_size_hints(cls[c], geom.width,
                                                 geom.height, useless_gap)
...
function twopane.left.arrange(p) return do_twopane(p, "left") end
...
function twopane.bottom.arrange(p) return do_twopane(p, "bottom") end
...
function twopane.top.arrange(p) return do_twopane(p, "top") end
