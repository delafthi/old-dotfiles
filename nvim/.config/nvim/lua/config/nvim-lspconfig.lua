...
function M.config()
  local ok, lspconfig = pcall(function() return require('lspconfig') end)
  ...
  local on_attach = function(client, bufnr)
    ...
    u.bufmap(bufnr, 'n', 'gp', '<Cmd>lua vim.lsp.diagnostic.goto_prev()<Cr>',
             opts)
    u.bufmap(bufnr, 'n', 'gn', '<Cmd>lua vim.lsp.diagnostic.goto_next()<Cr>',
             opts)
    u.bufmap(bufnr, 'n', 'gi', '<Cmd>lua vim.lsp.buf.implementation()<Cr>', opts)
    u.bufmap(bufnr, 'n', '<C-k>', '<Cmd>lua vim.lsp.buf.signature_help()<Cr>',
             opts)
    u.bufmap(bufnr, 'i', '<C-k>', '<Cmd>lua vim.lsp.buf.signature_help()<Cr>',
             opts)
    u.bufmap(bufnr, 'n', '<Leader>wa',
             '<Cmd>lua vim.lsp.buf.add_workspace_folder()<Cr>', opts)
    u.bufmap(bufnr, 'n', '<Leader>wr',
             '<Cmd>lua vim.lsp.buf.remove_workspace_folder()<Cr>', opts)
    u.bufmap(bufnr, 'n', '<Leader>wl',
             '<Cmd>lua print(vim.inspect(vim.lsp.buf.list_workspace_folders()))<Cr>',
             opts)
    u.bufmap(bufnr, 'n', '<Leader>D',
             '<Cmd>lua vim.lsp.buf.type_definition()<Cr>', opts)
    u.bufmap(bufnr, 'n', '<Leader>rn', '<Cmd>lua vim.lsp.buf.rename()<Cr>', opts)
    u.bufmap(bufnr, 'n', '<Leader>ld',
             '<Cmd>lua vim.lsp.diagnostic.show_line_diagnostics()<Cr>', opts)
    u.bufmap(bufnr, 'n', '<Leader>q',
             '<Cmd>lua vim.lsp.diagnostic.set_loclist()<Cr>', opts)

    -- Set some keybinds conditional on server capabilities
    if client.resolved_capabilities.document_formatting then
      u.bufmap(bufnr, 'n', '<Leader>bf',
               '<Cmd>lua vim.lsp.buf.formatting()<Cr>', opts)
    elseif client.resolved_capabilities.document_range_formatting then
      u.bufmap(bufnr, 'v', '<Leader>bf',
               '<Cmd>lua vim.lsp.buf.ranger_formatting()<Cr>', opts)
    end
  ...
  lspconfig.hls.setup {
    ...
    on_attach = on_attach,
    root_dir = lspconfig.util.root_pattern('*.cabal', 'stack.yaml',
